...
| 2026-08-12 | 디자인 고도화는 허브가 아니라 갤러리·서브앱 쪽 | 허브는 v2~v13까지 모션이 누적돼 이미 포화(커스텀 커서·베일 전환·⌘K·캔버스 2종). 허브엔 추가 대신 정리가 필요. 실제 격차는 갤러리 5개와 pitch-trainer/quotes의 브랜드 이탈 |
| 2026-08-12 | `.page` 에 animation-fill-mode:both 금지 | both fill이 opacity 애니메이션을 영구히 남겨 #page-hub를 합성 레이어로 승격 → #mkDust/#mkMoss 고정 캔버스가 콘텐츠 위로 올라와 페이지가 점점 검어짐. will-change:transform 금지와 같은 스태킹 컨텍스트 오염 계열 |
| 2026-08-12 | 외부 AI 산출물은 계약 검증 후 병합 | Manus 패치는 최소 침습이었으나 존재하지 않는 변수명 참조 + 블랙아웃 회귀 포함. MKPics 키/프레임 ID/origin 체크/블롭 해시 대조를 병합 전 필수 절차로 |
