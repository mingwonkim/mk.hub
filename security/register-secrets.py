...
def main():
    ...
    print('이메일 인증은 Firebase 발송을 사용합니다. 네이버 앱 비밀번호는 필요 없습니다.')
    if not registered('VAULT_GITHUB_TOKEN'):
        ...
        if not token:
            raise RuntimeError('GitHub 토큰 입력이 없어 중단했습니다. 다시 실행해 등록할 수 있습니다.')
    ...
    (ROOT / '.secret-registration-status.json').write_text(json.dumps({'registered': ['VAULT_GITHUB_TOKEN'], 'at': datetime.now(timezone.utc).isoformat()}))
