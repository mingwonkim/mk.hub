...
- [x] 시계 방향 점등 → **점진 와이프로 개선** — background-clip:text 그라디언트 + `--ckP`(0~100) 변수로 마우스 X를 따라 주황이 좌→우 채워지고 우→좌 지워짐. 경계 ±5% 페더, @property 등록으로 .18s 보간, 96↑/4↓ 스냅. 방향 semantics: dx>0이면 max(채움), dx<0이면 min(지움) — 흰 상태에서 오른쪽 진입 시 오점등 없음 (30/60/100/50/0/40 진행값 + 55% 렌더 실측)
